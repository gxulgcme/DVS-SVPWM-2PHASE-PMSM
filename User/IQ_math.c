...
const int16_t Sin_Cos_Angle_Q15_Table[256] = { \
...
const int32_t Atan_Angle_Q152_Table[256] = { \
...
void  Sin_Cos_Angle_Q15_Cale(p_Sin_Cos_Angle_Q15  pV)
{
	uint16_t  hindex;
	hindex = (uint16_t)pV->Angle_Q16; //-32768--- 32767 +32768===0--65535
	...
	{
	case U0_90:
		pV->Sin_Angle_Q15 = Sin_Cos_Angle_Q15_Table[(uint8_t)(hindex)];  // 0---255  ==0---32766
		pV->Cos_Angle_Q15 = Sin_Cos_Angle_Q15_Table[(uint8_t)(0xFF - (uint8_t)(hindex))];
		break;

	case U90_180:
		pV->Sin_Angle_Q15 = Sin_Cos_Angle_Q15_Table[(uint8_t)(0xFF - (uint8_t)(hindex))]; // 255---0 == 0---32766 
		pV->Cos_Angle_Q15 = -Sin_Cos_Angle_Q15_Table[(uint8_t)(hindex)];
		break;

	case U180_270:
		pV->Sin_Angle_Q15 = -Sin_Cos_Angle_Q15_Table[(uint8_t)(hindex)];
		pV->Cos_Angle_Q15 = -Sin_Cos_Angle_Q15_Table[(uint8_t)(0xFF - (uint8_t)(hindex))];
		break;

	case U270_360:
		pV->Sin_Angle_Q15 = -Sin_Cos_Angle_Q15_Table[(uint8_t)(0xFF - (uint8_t)(hindex))];
		pV->Cos_Angle_Q15 = Sin_Cos_Angle_Q15_Table[(uint8_t)(hindex)];
		break;
	default:
		break;
...
void  Atan_Angle_Q15_Cale(p_Atan_Angle_Q15 pV)  // 求取求反正弦函数
{
	...
	{
		pV->Tan_Angle_Q15 = (int32_t)(((int64_t)(Abs(pV->Beta)) << 15) / (int64_t)(Abs(pV->Alpha)));
		if (Atan_Angle_Q152_Table[i + 128] <= pV->Tan_Angle_Q15) i += 128;
		if (Atan_Angle_Q152_Table[i + 64] <= pV->Tan_Angle_Q15) i += 64;
		if (Atan_Angle_Q152_Table[i + 32] <= pV->Tan_Angle_Q15) i += 32;
		if (Atan_Angle_Q152_Table[i + 16] <= pV->Tan_Angle_Q15) i += 16;
		if (Atan_Angle_Q152_Table[i + 8] <= pV->Tan_Angle_Q15) i += 8;
		if (Atan_Angle_Q152_Table[i + 4] <= pV->Tan_Angle_Q15) i += 4;
		if (Atan_Angle_Q152_Table[i + 2] <= pV->Tan_Angle_Q15) i += 2;
		if (Atan_Angle_Q152_Table[i + 1] <= pV->Tan_Angle_Q15) i += 1;
...
#define PI_HALF     1.5707963268f     // PI/2
...
static const float sin_table[TABLE_SIZE] = {
	0.000000f, 0.017453f, 0.034899f, 0.052335f, 0.069756f, 0.087155f, 0.104528f, 0.121869f,
	...
	0.406737f, 0.422618f, 0.438371f, 0.453990f, 0.469469f, 0.484810f, 0.500000f, 0.515038f,
	...
	0.469469f, 0.453990f, 0.438371f, 0.422618f, 0.406737f, 0.390731f, 0.374607f, 0.358368f,
	0.342020f, 0.325568f, 0.309017f, 0.292372f, 0.275637f, 0.258819f, 0.241922f, 0.224951f,
	0.207912f, 0.190809f, 0.173648f, 0.156434f, 0.139173f, 0.121869f, 0.104528f, 0.087155f,
	0.069756f, 0.052335f, 0.034899f, 0.017453f, 0.000000f, -0.017453f, -0.034899f, -0.052335f,
	-0.069756f, -0.087155f, -0.104528f, -0.121869f, -0.139173f, -0.156434f, -0.173648f, -0.190809f,
	...
	-0.469469f, -0.484810f, -0.500000f, -0.515038f, -0.529919f, -0.544639f, -0.559193f, -0.573576f,
	...
	-0.529919f, -0.515038f, -0.500000f, -0.484810f, -0.469469f, -0.453990f, -0.438371f, -0.422618f,
	...
	-0.139173f, -0.121869f, -0.104528f, -0.087155f, -0.069756f, -0.052335f, -0.034899f, -0.017453f
};

/**
 * @brief  手工实现角度归一化到 [0, 360)，替代 fmodf
 * @param  deg: 角度值
 * @retval 归一化后的角度
 */
static float normalize_deg(float deg)
{
	// 减去整数倍的360，把角度限制在 0~360 之间
...
float sinf(float rad)
{
	...
	// 索引计算 + 插值
	float index_f = deg / DEG_STEP;
	int index = (int)index_f;
	float frac = index_f - index;

	int next_index = (index + 1) % TABLE_SIZE;
	return sin_table[index] + frac * (sin_table[next_index] - sin_table[index]);
}
