...
void CMP_init(void)
{
  ...
  CMP_InitStruct.CMP1_SELP = CMP1_SELP_CMP1_IP0;
...
void DAC_init(void)
{
  DAC_InitTypeDef DAC_InitStructure;
	...
	DAC_InitStructure.DACOUT_EN  = ENABLE;
...
void ADC_init(void)
{
  ...
  ADC_InitStructure.Align = ADC_RIGHT_ALIGN;//ADC_LEFT_ALIGN;
...
void GPIO_init(void)
{
  ...
  GPIO_PinAFConfig(GPIO0, GPIO_PinSource_15, AF3_MCPWM);
    ...
    GPIO_InitStruct.GPIO_Pin = GPIO_Pin_0 | GPIO_Pin_7 | GPIO_Pin_8;
    GPIO_InitStruct.GPIO_PuPd = GPIO_PuPd_UP; /*不上拉*/
    GPIO_Init(GPIO0, &GPIO_InitStruct);
	
//lzq    GPIO_PinAFConfig(GPIO0, GPIO_PinSource_0, AF3_MCPWM);
    MCPWM_EIF = BIT4; //清Fail0事件标志        
/*
...
void MCPWM_init(void)
{
  ...
  MCPWM_InitStructure.FAIL1_INPUT_EN = DISABLE;             /* FAIL1通道急停功能打开或关闭 ENABLE*/
  ...
  MCPWM_InitStructure.FAIL0_INPUT_EN = DISABLE;             /* FAIL1通道急停功能打开或关闭  ENABLE */
  MCPWM_InitStructure.FAIL0_Signal_Sel = FAIL_SEL_IO;      /* FAIL1事件信号选择，比较器或IO口 */
  MCPWM_InitStructure.FAIL0_Polarity = LOW_LEVEL_VALID;           /* FAIL0事件极性选择，高有效或低有效 */
  ...
  //lzq mIPD_CtrProc.hDriverPolarity = MCPWM_IO01;                /* 读出驱动极性 */
}
